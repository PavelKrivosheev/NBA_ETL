...
from load_incremental_game import load_incremental_game
...
with DAG(
    ...
    check_status = PythonOperator(
        task_id='check_load_status_game',
        python_callable=check_load_status,
        op_args=['game'],
        provide_context=True
    )
    load_task = PythonOperator(
        task_id='load_data_game',
        python_callable=load_incremental_game,
        op_args=['/opt/airflow/data/nba.sqlite', '{{ ti.xcom_pull(key="next_year_game") }}', 'dag_load_incremental_game', 'load_data_game'],
        provide_context=True
    )
    update_progress = PythonOperator(
        task_id='update_progress_game',
        python_callable=update_load_progress,
        op_args=['game'],
        provide_context=True
    )
    trigger_next = TriggerDagRunOperator(
        task_id='trigger_dag_load_incremental_line_score_game',
        trigger_dag_id='dag_load_incremental_line_score_game'
    )
