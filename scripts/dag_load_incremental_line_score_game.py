...
from load_incremental_line_score_game import load_incremental_line_score_game
...
with DAG(
    'dag_load_incremental_line_score_game',
    description='DAG для переноса данных об играх из SQLlite в stage.',
    ...
    check_status = PythonOperator(
        task_id='check_load_status_line_score_game',
        python_callable=check_load_status,
        op_args=['line_score_game'],
        provide_context=True
    )
    load_task = PythonOperator(
        task_id='load_data_line_score_game',
        python_callable=load_incremental_line_score_game,
        op_args=['/opt/airflow/data/nba.sqlite', '{{ ti.xcom_pull(key="next_year_line_score_game") }}', 'dag_load_incremental_line_score_game', 'load_data_line_score_game'],
        provide_context=True
    )
    update_progress = PythonOperator(
        task_id='update_progress_line_score_game',
        python_callable=update_load_progress,
        op_args=['line_score_game'],
        provide_context=True
    )
    trigger_next = TriggerDagRunOperator(
        task_id='trigger_dag_load_incremental_shot_data',
        trigger_dag_id='dag_load_incremental_shot_data'
    )
